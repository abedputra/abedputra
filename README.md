# 👋 Hi, I'm Abed Putra

## 🧠 Senior Backend Engineer | 🏗 System Architect

I design and build scalable backend systems, high-performance REST APIs, and production-ready applications focused on reliability, maintainability, and long-term growth.

---

## 👨🏻‍💻 Professional Summary

- 💼 8+ years of experience building commercial-grade software
- 🧩 Strong expertise in backend architecture & API design
- 🌍 Experience delivering systems used by paying customers globally
- ⚙️ Focused on clean architecture, scalability, and performance
- 📚 Currently exploring Golang and distributed systems
- 📫 Contact: **abedputra@gmail.com**

---

## 🚀 Selected Engineering Work

### 🧾 Smart Absens — Workforce Management SaaS  
Architected and developed a scalable attendance & workforce management platform supporting:

- 🔐 Multi-authentication (Fingerprint, QR, PIN, Face Recognition)
- ⚡ Real-time attendance processing
- 🛡 Role-based access control
- ☁️ Production deployment & infrastructure optimization
- 📊 Scalable database & API architecture

🌐 Live platform: https://smartabsens.com/

---

### 🏢 Commercial Business Systems  
Designed and delivered Laravel-based business platforms used by paying customers worldwide.

**Engineering focus:**

- 🧱 Clean architecture implementation
- 🔗 REST API design & third-party integrations
- 🔐 Secure authentication & authorization systems
- 🚀 Performance optimization & production deployment

🔗 Portfolio: https://codecanyon.net/user/muliatech/portfolio

---

## 📈 Engineering Impact

- 💰 273+ verified commercial software sales
- ⭐ 4.8 / 5 average product rating
- 🌎 Systems used by businesses across multiple countries
- 🧠 Designed with scalable backend architecture principles

---

## 🧰 Technical Stack

### 🖥 Backend
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/Laravel-EF3B2D?style=flat&logo=laravel&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat&logo=go&logoColor=white)

### 📱 Mobile
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white)

### 🗄 Database & APIs
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![REST API](https://img.shields.io/badge/REST-API-success)

---

## 🏆 Recognition

### 💬 Stack Overflow
- 🥇 2 Gold Badges  
- 🥈 21 Silver Badges  
- 🥉 41 Bronze Badges  
🔗 https://stackoverflow.com/users/5887075/abed-putra

### 🏅 PHPClasses
- 🏆 Innovation Award Winner  
- 🥇 1x Winner  
- 🏅 6x Nominee  
🔗 https://www.phpclasses.org/browse/author/1399477.html

---

## 🌐 Connect

- 🌍 Website: https://abedputra.my.id  
- 💼 LinkedIn: https://www.linkedin.com/in/abed-putra/  

---

## 🤝 Open to Engineering Collaboration

Interested in backend architecture, scalable systems, or SaaS engineering?  
Let’s connect.
